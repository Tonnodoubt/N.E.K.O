...
def detect_meld_state_path(
    ...
    *,
    ...
    with Image.open(image_path) as opened:
        ...
        parsed = parse_self_melds_from_image(image, min_confidence=min_confidence)
...
def build_self_meld_layout(width: int, height: int) -> list[MeldSlot]:
    ...
    tile_height = max(60, int(tile_width * 1.55))
    if aspect_ratio >= 2.35:
        tile_height = max(tile_height, int(screen_height * 0.18))
        top_ratio = 0.61
    elif screen_height <= 720:
        tile_height = max(tile_height, int(screen_height * 0.23))
        top_ratio = 0.68
    else:
        top_ratio = 0.70
    top = _clamp_int(round(screen_height * top_ratio), 0, screen_height - tile_height)
    left = _clamp_int(round(screen_width * 0.075), 0, screen_width - tile_width)
    step = max(18, int(tile_width * 0.72))
    slots: list[MeldSlot] = []
    max_slots = 16
    for index in range(max_slots):
        slot_left = left + index * step
        if slot_left + tile_width > int(screen_width * 0.45):
            break
        meld_index = min(MAX_SELF_MELDS, 1 + index // 4)
        tile_index = 1 + index % 4
        box = RoiBox(
...
def _cluster_meld_tiles(accepted: list[dict[str, Any]], _screen_width: int) -> list[dict[str, Any]]:
    ...
    for item in accepted:
        try:
            meld_index = int(item.get("meld_index") or 0)
        except (TypeError, ValueError):
            meld_index = 0
        if meld_index > 0:
            grouped[meld_index].append(item)

    melds: list[dict[str, Any]] = []
    for index in sorted(grouped)[:MAX_SELF_MELDS]:
        cluster = sorted(grouped[index], key=lambda value: int(value.get("tile_index") or 0))
...
def _is_probably_occupied_meld_slot(slot_metrics: dict[str, Any]) -> bool:
    ...
    return mean_luma >= 82.0 and bright_ratio >= 0.10 and dark_ratio <= 0.68 and stddev >= 12.0
