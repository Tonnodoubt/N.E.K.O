...
class LLMSessionManager:
    ...
    def _start_tts_thread(self, audio_format=None):
        """创建并启动 TTS Worker 线程。
        ...
        self._last_audio_format = audio_format
    ...
    def _respawn_tts_worker(self):
        ...
        self._start_tts_thread(audio_format=getattr(self, 'audio_format', None))
    ...
    async def start_session(self, websocket: WebSocket, new=False, input_mode='audio', audio_format=None):
        ...
        async def start_tts_if_needed():
            ...
            if self.tts_thread is None or not self.tts_thread.is_alive():
                self._start_tts_thread(audio_format=self.audio_format)
